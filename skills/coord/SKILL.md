...
Built-in roles are inherited by exact names and by suffixed session names. `reviewer-*` / `reviewer_*`, `executor-*` / `executor_*`, `frontend-*` / `frontend_*`, `backend-*` / `backend_*`, `planner-*` / `planner_*`, and `stabilizer-*` / `stabilizer_*` automatically inherit the matching built-in role profile. Use this for extra live sessions, such as `executor-2` or `executor_hotfix`, instead of reusing `executor`.

Built-in role names:

- `reviewer`: reviews spec, plan, code, tests, and delivery results. In spec/plan phases, check requirement completeness, consistency, risks, acceptance criteria, and execution clarity. In code phases, check bugs, regressions, API contracts, test gaps, maintainability, and whether user requirements are met. For spec/plan, UI/interaction, product-scope, engineering-direction, devex, or PR-like reviews, use applicable gstack review skills when available: `plan-ceo-review`, `plan-design-review`, `plan-eng-review`, `plan-devex-review`, `design-review`, `devex-review`, or `review`. If applicable gstack skills are unavailable or undiscoverable for a review that would benefit from them, state the limitation and record residual review risk. Record gate verdicts for every review, including no-issue reviews: `verdict=approved` or `verdict=changes_requested`, reviewed artifacts, blocking issues, residual risk, and allowed next stage. For UI/interaction implementation reviews, check actual rendered behavior against the `UI/交互需求说明` using browser, screenshot, or manual evidence where possible. Do not edit files by default unless the user explicitly asks.
- `executor`: implements approved plans, reviewer-approved fixes, or direct user requests when no separate plan is needed. Do not write or redesign the spec/plan by default; ask `planner` or the user when the approved plan is missing, stale, or contradicted by new requirements. Implement, fix, test, and verify while staying aligned with the latest approved spec/plan and reviewer gate verdict. Stop for confirmation when requirements change, plans conflict, cross-agent dependencies appear, or risky operations are needed. Record implementation handoff before review: spec/plan paths, changed files, implementation summary, verification run, verification not run with reasons, deviations, UI/interaction evidence, and remaining risk.
...
- `planner`: plans solution, experience, and engineering direction before implementation. MUST use applicable gstack skills when available: `office-hours`, `plan-ceo-review`, `design-consultation`, `plan-design-review`, `plan-eng-review`, and `plan-devex-review`. If applicable gstack skills are unavailable or undiscoverable, state that limitation before planning and record the gap in coord. Write or update superpowers specs and implementation plans from confirmed user requirements, reviewer feedback, or approved planning conclusions. Use `superpowers:brainstorming` for specs, including `UI/交互需求说明` before technical design for UI or interaction work. Use `superpowers:writing-plans` after spec approval, or when the user explicitly wants spec and plan drafted together. Do not implement code by default. Record artifact paths and `ready_for_review` status after writing or updating specs/plans; reviewer records the phase gate verdict.
- `stabilizer`: stabilizes work after implementation review and before final delivery. Help the user test, reproduce issues, investigate root cause, fix bugs, and re-run verification until the work is ready to ship or a blocker needs user decision. Start from coord `brief`/`sync` plus the actual spec, plan, code, tests, and latest reviewer gate verdict; do not rely only on the latest handoff. Use systematic debugging for bugs and keep fixes aligned with the latest approved spec/plan and `UI/交互需求说明`. Record final deliverable status: tests performed, bugs fixed, verification evidence, known residual risk, and unresolved follow-up.

`planner` requires gstack to be installed and discoverable for planning work, and superpowers to be installed and discoverable for spec/plan writing. `reviewer` should use gstack review skills when they apply. Coord can still record these roles without those dependencies, but the affected workflow cannot be executed correctly until the required skills are available.
...
Stage handoff defaults:

- Authors record artifacts and changes. `planner` records spec/plan paths and `status=ready_for_review`; `executor` records implementation handoff before code review.
- Reviewers record gates. Every reviewer pass records `verdict=approved` or `verdict=changes_requested`, reviewed artifacts, blocking issues, residual risk, and the allowed next stage, including no-issue reviews.
- `stabilizer` records the final deliverable state after testing and bug fixing.
- A prior handoff is not immutable. If later user requirements supersede a handoff or approved artifact, record the new effective state and use `correct`, `retract`, or `impact` according to the correction rules above. If the old record may already have influenced another agent, prefer `impact`.
...
- "认领 ..." -> claim.
- "交接一下" -> handoff.
- "`<agent>` 已完成" -> sync, inspect the latest effective note/handoff/decision/claim activity from that agent, then continue according to the current role. A reviewer should review the completed work, an executor should apply required changes or report no action needed, and a stabilizer should continue testing, bug fixing, or final verification when appropriate.
